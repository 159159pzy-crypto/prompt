...
本地单用户 Anima 图像提示词生成工作区（**Prompt Workbench v7**）。用户用中文描述画面，模型按自然语言要求返回任意数量的结构化正面 Token，并可按相同位置返回精确中文对照。生成任务由本地 SQLite 队列和独立 Worker 执行，支持刷新恢复、取消、重试和幂等提交。
...
打开 `http://127.0.0.1:8191`。首次启动会创建 `data/workbench.sqlite3`。升级到 schema v4 时只重建运行历史，不删除文档、供应商、设置或密钥引用。升级到 schema v5 时只追加 `conversations` 表和文档 provenance 列，**不重建运行历史**。文档写入已改为具名列；若把代码退回 v6 而数据库留在 v5，文档写入会失败，必须恢复升级前的 SQLite 拷贝。建议升级或硬删对话前复制 `data/workbench.sqlite3` 到 `data/backups/`。

## 工作流

1. 在输出结果页直接输入自然语言意图；可写“生成 5 组服装变体”，未写数量时默认 1 组。
2. 在模型与路由中选择 OpenAI-compatible 供应商、模型和思考强度；供应商页可新增、导入、测通和同步模型。
3. 在语言设置中决定是否请求逐 Token 中文对照；多组或变体请求会按语义选择并注入相关 Skills，Agent 仍可按需调用只读工具。
4. 在系统提示词设置中编辑最多 12,000 字符的本地规则；留空时使用默认 Anima 系统提示词。
5. 生成后可复制英文正面 Prompt、显式「保存为文档」，或从文档库「从文档恢复为修改基线」。侧栏可搜索、重命名、置顶、分页加载对话；删除对话会硬删该对话下的运行记录，已保存文档保留。
...
供应商页可新增、编辑、导入、测通和同步模型；密钥保存在 Windows 凭据存储，界面在保存后不再显示。文档库与对话 API 已接到工作台 UI。
...
Agent 使用固定 `Planner -> Generator -> Validator/Repair -> Finalizer` 流水线和 OpenAI Chat Completions `tools`/`tool_calls` 协议。API 采用 Run + 轮询，`/api/generate` 保留为兼容接口。运行事件保存在 schema v5 的 `agent_events` 表中，并通过 Run events API 查询；Worker 在 `execute_run` 循环内续约 lease，并在进程重启后恢复未完成任务。`POST /api/generate` 与队列 Run 走同一条 `execute_run` 流水线。
